...
const Work = () => {
  ...
  // Load projects from CMS data
  useEffect(() => {
    const loadProjects = () => {
      const cmsProjects = getPublishedProjects();
      if (cmsProjects.length > 0) {
        // Convert CMS format to Work page format
        const convertedProjects = cmsProjects.map((project, index) => ({
        ...
        // Fall back to default projects if no CMS data
    ...
    const unsubscribe = onPortfolioUpdate(() => {
      loadProjects();
    });
    ...
    <div className="min-h-screen bg-charcoal text-white">
      ...
      <section className="relative py-20 px-8 bg-gradient-to-b from-charcoal to-charcoal/80 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          {/* Admin Access Button and CMS Status */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 1 }}
          ...
          >
            ...
            <div className="flex items-center gap-1 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-xs">
              <div
            ...
            <motion.button
              ...
              className="opacity-0 hover:opacity-100 transition-opacity duration-300 p-2 text-gray-500 hover:text-beige text-xs bg-black/50 backdrop-blur-sm rounded-full"
              title="Admin Access (Ctrl+A)"
              onKeyDown={(e) => {
